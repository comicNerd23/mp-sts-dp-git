...
/**
 * Dependency inversion principle states: Entities must depend on abstractions, not on concretions. It states that the 
 ...
 * Later, if you were to change the database engine, you would also have to edit the PasswordReminder class, and this 
 * would violate the open-closed principle.
 *	
 * The PasswordReminder class should not care what database your application uses. To address these issues, you can code 
 * to an interface since high-level and low-level modules should depend on abstraction:
 */
